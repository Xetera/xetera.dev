...
function PacketExplorer(props: { data: IndividualPacket; index: number }) {
  ...
  return (
    <div className="text-xs md:text-sm ms-4 font-mono mb-2">
      ...
      {layers.pap && <PapLayer packet={layers.pap} />}
...
function PapLayer({ packet, expanded }: Layer<"pap">) {
  return (
    <Details>
      <summary>PPP Password Authentication Layer</summary>
      <Items>
        <span>Code: Authentication Request ({packet["pap.code"]})</span>
        <span>Identifier: {packet["pap.identifier"]}</span>
        <span>Length: {packet["pap.length"]}</span>
        <Details>
          <summary>Data</summary>
          {packet["pap.code"] === "1" && (
            <Items>
              <span>
                Peer-ID-Length: {packet["pap.data"]["pap.peer_id.length"]}
              </span>
              <span>Peer-ID: {packet["pap.data"]["pap.peer_id"]}</span>
              <span>
                Password-Length: {packet["pap.data"]["pap.password.length"]}
              </span>
              <span>Password: {packet["pap.data"]["pap.password"]}</span>
            </Items>
          )}
          {packet["pap.code"] === "3" && (
            <Items>
              <span>
                Message-Length: {packet["pap.data"]["pap.message.length"]}
              </span>
              <span>Message: {packet["pap.data"]["pap.message"]}</span>
            </Items>
          )}
        </Details>
      </Items>
