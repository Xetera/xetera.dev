...
export default function Timeline(props: Props) {
  ...
  const currentHovering = props.selected !== undefined ? props.shows[props.selected] : undefined
  ...
  return (
    <div className="relative flex items-center">
      ...
      {currentWatched && <p className="text-sm absolute left-50% -translate-x-50% bottom-4">Watched {currentWatched}</p >}
        ...
        props.shows.toReversed().map((show, i, arr) => {
          ...
          return (
            <div
              key={show.simklId}
              style={{ right: `${offset}%` }}
              className={cls("transition-all w-3 h-3 absolute right-0 rounded", hovering ? "bg-text-700" : "bg-body-300")
              }
            />
          );
        })
