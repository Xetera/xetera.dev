...
I've also made a small patcher to automate all of the setup discussed, as well as fix weird bugs about the game that require patching the executable.

![A screenshot showing the list of patches available in the published desktop application](image.png)

Please take a look and play a game for yourself. Because there are a total of like 4 people including myself who still play this game, Caesar's Challenges are active for an entire year instead of the 2 weeks they were originally implemented as.

---

† _I'm interested in working on an open-source reimplementation of Caesar IV, similar to the Julius project. Sadly, the game is built with C++, using a newer compiler with modern optimizations (that [reccmp](https://github.com/isledecomp/reccmp) does not officially support), as well as external dependencies. This is going to be much more difficult project than the original Julius project, but I'm interested in giving it a shot by letting a bunch of AI agents loose on the problem to see if I can get an identical binary output and make the game truly cross-platform compatible._
