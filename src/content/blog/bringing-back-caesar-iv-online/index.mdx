...
The game makes raw SQL queries! Not the multiplayer server, to be clear, the game client itself. After looking through the decompiled code, it's clear there's no multiplayer server at all. It was all just an online leaderboard people could post their scores on by adding rows to a MySQL database directly from the client. To be fair, they probably did not give `delete` or `drop` privileges to the connecting user, considering the database didn't blow up until they shut it down on purpose. And they likely moderated the leaderboards to remove obvious cheaters (if there were any). There is a concept of a "banned" user that the client respects, but it seems like all copies came with a hardcoded user, so how could you _really_ ban someone with that setup?

The Steam release replaces the leaderboard link with `activision.com`, but I was able to find the original link by going through the Caesar 4 Heaven <ExternalLink href="https://caesar4.heavengames.com/archives/december-2007/">archived news posts from 2007</ExternalLink>. Plugging that into the [Wayback Machine](https://web.archive.org/web/20260000000000*/http://caesarivonline.com), we can see there was a website dating back to 2006 that was, uh, no doubt made in 2006.

![A very old looking website www.caesarivonline.com showing a leaderboard of people's scores](./online_leaderboards.png)
...
![An image showing "Online services are no longer supported." when logging into Caesar IV Online](./online_broken.png)
...
As you load up a scenario, you will notice new UI elements you haven't seen before. This might actually be the first ever screenshot of the online play.
...
<WideMedia contain>
  <div class="flex flex-col w-full">
    <WidePicture
      alt="An in-game leaderboard of the Caesar's Challenge in a game of Caesar IV Online, with xetera as #1 governor"
...
![An SVG minimap of c4.xetera.dev showing buildings overlaid on top of the minimap the way they're built in the game](minimap.png)

This is more or less what <T>Caesar IV Online</T> is: a leaderboard you tried to reach the top of before it reset two years later. I've decided to take everything I learned here to revive a new leaderboard at <ExternalLink
target="_blank"
referrer
class="text-blue-400 underline"
href="https://c4.xetera.dev/?utm_source=blog">https://c4.xetera.dev</ExternalLink> (the original domain is sadly squatted).

![A preview of the new leaderboards with just xetera on it.](image.jpg)

Unlike the original game, you won't be connecting directly to a MySQL database. I've set up a Go webserver to proxy connections without `delete`/`drop` permissions, whitelist only the queries the game makes, and check to make sure users only send requests for the username they've logged in as. Caesar's Challenges are active for an entire year instead of the 2 weeks they were originally implemented with. Sign up for an account (no email verification required), run the patcher and get something up on the leaderboard!

![A screenshot showing the list of patches available in the published desktop application](image.png)

<Caption>
  The patcher automates all of the setup discussed, as well as fixing weird bugs
  in the game that require access to the executable.
</Caption>

I know there's virtually nobody playing or thinking about this game in 2026; you can't even run it on Steam without going through the community posts for workarounds (buy it on [GOG](https://www.gog.com/en/game/caesar_iv) instead). But I know there are at least a few people like me out there who had fond memories of playing it as a kid, and now you can at least try something you likely didn't even know existed back when times were simpler.

---

_PS: I'm interested in working on an open-source reimplementation of Caesar IV, similar to the Julius project. Sadly, the game is built with C++, using a newer compiler with modern optimizations (that [reccmp](https://github.com/isledecomp/reccmp) does not officially support), as well as external dependencies. This is going to be a much more difficult project than the original Julius project, but I'm interested in giving it a shot by letting a bunch of AI agents loose on the problem to see if I can get an identical binary output and make the game truly cross-platform compatible._
