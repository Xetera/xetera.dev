...
export async function books(): Promise<KindleBookDetails[]> {
	if (!process.env.GATSBY_COOKIES || !process.env.GATSBY_DEVICE_TOKEN) {
		console.debug(`Missing GATSBY_COOKIES or GATSBY_DEVICE_TOKEN env variables, skipping initialization of Kindle data`)
		return []
	}

	const kindle = await Kindle.fromConfig({
		cookies:
			process.env.GATSBY_COOKIES!,
		deviceToken: process.env.GATSBY_DEVICE_TOKEN
	});

	const out: KindleBookDetails[] = [];
